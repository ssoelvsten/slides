...
\blankframe
\section{Goal:\\Resource Reuse}

\begin{frame}
  \frametitle{Resource Reuse}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=2]
      ...
      \onslide<3-> {
        \draw[->] (A) edge[bend right] node[below] {$g_M$} (B);
      }
    \end{tikzpicture}
...
\begin{frame}
  \frametitle{Leaky Cache (Confidentiality)}
...
\begin{frame}
  \frametitle{Replay Dependency (Integrity)}

  \begin{figure}
    \centering\huge

    \begin{tikzpicture}
      \node[shape=rectangle, draw=black] at (0, 1)
      (A) {\texttt{A}};
      ...
      \draw[->]
        % A -> B
        (A) edge[bend left=10] node[above] {$g_{\texttt{auth}}$} (B)
      ;
      ...
      \onslide<2-> {
        ...
        \draw[->]
          % C -> B
          (M) edge[bend right=10] node[below] {$f_{\texttt{auth}}$} (B)
        ;
      }
    \end{tikzpicture}
...
\begin{frame}
  \frametitle{Chain of Death (Availability)}
...
\section{Requirements on Peer-To-Peer Cache}

\begin{frame}
  \frametitle{Requirement: Small Trusted-Computing Base (TCB)}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      \node[draw=black] at (2,0) (A-p2p) {\texttt{p2p.ts}};

      \node at (8,1) {\Large Bob};
      \node[draw=black] at (7,0) (B-p2p) {\texttt{p2p.ts}};
...
\begin{frame}
  ...
  Changes to the Troupe runtime (\texttt{.ts}):
  \begin{itemize}
  \item To \texttt{deserialize}, (1) extract references (2) deserialize given
    (all) dependencies.
  \item During \texttt{serialize}, identify values to be referenced by hash.
  \item Extend \texttt{Scheduler} to run Troupe code from \texttt{p2p} thread.
  \end{itemize}

  Addition of the cache in Troupe itself (\texttt{.trp}):
  \begin{itemize}
  \item Design a \texttt{HashMap} and \texttt{Map} in Troupe's standard library.
  \end{itemize}
\end{frame}

\begin{frame}[t]
  \frametitle{Requirement on \texttt{HashMap} (Level View)}

  \begin{itemize}
  \item A single data structure with public and secret key-value pairs.

  \item Secret key-value pairs should not taint public ones.

  \item Key-value pairs should be accessible across levels.
    \only<1-4> {
    \begin{align*}
      \onslide<2-4> {
      ...
      \onslide<4-4> {
      & D &\xrightarrow{m@\{D\}} B \implies \text{hit}
      }
    \end{align*}
    }
  \end{itemize}
  ...
  \begin{center}
    \begin{tabular}{cccccc}
      \texttt{i($x \mapsto 1$)@\{\}}, &
      \only<6,8>{\color{lightgray}}\texttt{i($x \mapsto 2$)@\{A\}}, &
      \only<6-7>{\color{lightgray}}\texttt{i($y \mapsto 3$)@\{B\}}, &
      \only<6-7>{\color{lightgray}}\texttt{d($x$)@\{B\}}, &
      \texttt{i($y \mapsto 4$)@\{\}}, &
      $\dots$
    \end{tabular}
    ...
    { \large
      \only<5>{\phantom{\texttt{[$y$] @\{\}}}}%
      \only<6>{\texttt{[$x \mapsto 1$@\{\}, $y \mapsto 4$@\{\}] @\{\}}}%
      \only<7>{\texttt{[$x \mapsto 2$@\{A\}, $y \mapsto 4$@\{\}] @\{A\}}}%
      \only<8>{\texttt{[$y \mapsto 4$@\{\}] @\{B\}}}%
    }
  \end{center}
  }
\end{frame}
...
% - Current prototype is with a Hash Array-Mapped Trie
%   - But, this requires too much declassification
% - Alternative (meta?) data structure
%   - IFC levels are first class
%      => prior to the key, sort on (partially ordered) access levels
%   - Buffers to ensure (amortized?) cheap `insert` and `delete`
%   - But, how to keep `find` cheaply if the same key at different levels is not
%     centralized?

\section{Design of the \texttt{HashMap}}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\linewidth}
      Current prototype uses a Hash Array Mapped Trie (HAMT).

      \bigskip

      Only 3 declassifications per \texttt{insert}, \texttt{remove}, \texttt{find}:
      \begin{itemize}
      \item \texttt{hash key} and \texttt{hash bucket} to not raise \texttt{pc} and taint the entire
        trie.
...
\begin{frame}
  \frametitle{Design of the \texttt{HashMap}}
    ...
    \begin{columns}
    \begin{column}{0.5\linewidth}
      \textbf{Future Work:} Meta data structure for \texttt{HashMap} and \texttt{Map}?
      ...
      But, does this make \texttt{find} too slow?
    \end{column}
    \begin{column}{0.49\linewidth}
      \centering
      ...
      \begin{tikzpicture}
        ...
        \onslide<4-> {
          \node[buffer] at (1.7,1) {\small $[i(k,v), d(k), \dots]$};
        }
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile]
  \frametitle{Mitigate Timing Attacks}

  Troupe provides the \texttt{{\color{blue} sandbox}($f$, $t$)} to run $f$ for
  $t$~ms and either return the result, an error, or a continuation. In practice,
  $t = 1$~ms is never exceeded.
