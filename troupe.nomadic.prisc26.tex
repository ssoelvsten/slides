...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=2]
      ...
      \onslide<2-> {
        \draw[-latex, thick] (A) edge node[above] {$f_{\texttt{Mod}}$} (B);
      }
      \onslide<3-> {
        \draw[-latex, thick] (A) edge[bend right=30] node[below] {$g_{\texttt{Mod}}$} (B);
      }
    \end{tikzpicture}
...
\begin{frame}[plain,noframenumbering]
  \begin{center}
    {
      \LARGE\bf
      How can one add a \only<1>{module}\only<2>{\sout{\color{lightgray}module}} cache?
    }

    \vspace{-32pt}
    \onslide<2->{\hspace{100pt}{\large\bf \textit{dependency}}}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=2]
      ...
      \onslide<2-> {
        \draw[-latex, thick] (A) edge node[above] {$f_{\texttt{Mod}}$} (C);
      }
      \onslide<3-> {
        \draw[-latex, thick] (C) edge node[above, pos=.1] {*} node[above] {$f_{\texttt{Mod}}$} (B);
      }
      \onslide<4-> {
        \draw[-latex, thick] (A) edge[bend right=30] node[below] {$g_{\texttt{Mod}}$} (B);
      }
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=2]
      ...
      \onslide<2-> {
        ...
        \draw[-latex, thick] (A) edge[bend left] node[above] {$f_{\texttt{Mod}}$} (B);
      }
      \onslide<3-> {
        \node at (0, -1) (Ad) {$\dagger$};
        \draw[-latex, thick] (A) edge[dashed] (Ad);
      }
      ...
      \onslide<4-> {
        ...
        \draw[-latex, thick] (B) edge[bend left] node[above] {$f_{\texttt{Mod}}$} (C);
      }
      \onslide<5-> {
        \node at (1, -1) (Bd) {$\dagger$};
        \draw[-latex, thick] (B) edge[dashed] (Bd);
      }
      ...
      \onslide<6-> {
        ...
        \draw[-latex, thick] (C) edge[bend left] node[above] {$f_{\texttt{Mod}}$} (D);
      }
      \onslide<7-> {
        \node at (2, -1) (Cd) {$\dagger$};
        \draw[-latex, thick] (C) edge[dashed] (Cd);
      }
      ...
      \onslide<8-> {
        ...
        \draw[-latex, thick] (Y) edge[bend left] node[above] {$f_{\texttt{Mod}}$} (Z);
      }
      \onslide<9-> {
        \node at (5, -1) (Zf) {$f_{\texttt{Mod}}()$};
        \draw[-latex, thick] (Z) edge[dashed] (Zf);
      }
    \end{tikzpicture}
...
\begin{frame}[plain,noframenumbering]
  \begin{center}
    ...
    {\color{gray}How can one add an} {IFC-aware} {\color{gray}cache?}
  \end{center}
\end{frame}
...
in loop ()
    ...
    \begin{column}{0.55\linewidth}
      ...
      \item \textbf{Information Flow Control}
        ...
        Ensures via a monitor that secret information is not by accident shared
        with nor affected by unwanted parties.
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      \onslide<2-> {
        ...
        \draw[-latex, thick] (AP-1) edge node[above] {\texttt{set}} (AC-1);
        \draw[-latex, thick] (AP-1) edge node[above] {\texttt{send}} (BP-1);
      }
      \onslide<3-> {
        \node[shape=circle, draw=black, fill=white] (BC-1) at (9,-1) {};
        \draw[-latex, thick] (BP-1) edge node[above] {\texttt{get}} (BC-1);
      }
      \onslide<4-> {
        \node[shape=circle, draw=black, fill=white] (BC-2) at (9,-2) {};
        \node[shape=circle, draw=black, fill=white] (BP-2) at (7,-2) {};

        \draw[-latex, thick] (BC-2) edge (BP-2);
      }
      \onslide<5-> {
        \node[shape=circle, draw=black, fill=white] (AP-2) at (2,-2) {};

        \draw[-latex, thick] (BP-2) edge node[above] {\texttt{miss}} (AP-2);
      }
      \onslide<6-> {
        ...
        \draw[-latex, thick] (AP-2) edge node[above] {\texttt{get}} (AC-2);
        \draw[-latex, thick] (AC-3) edge (AP-3);
        \draw[-latex, thick] (AP-3) edge node[above] {\texttt{miss reply}} (BP-3);
      }
      \onslide<7-> {
        \node[shape=circle, draw=black, fill=white] (BC-3) at (9,-3) {};
        \draw[-latex, thick] (BP-3) edge node[above] {\texttt{set}} (BC-3);
      }
    \end{tikzpicture}
...
\begin{frame}[fragile]
  ...
  \texttt{cache.trp} is only a tiny Troupe program that orchestrate a
  \texttt{HashMap} instance.
  \begin{itemize}
  \item<2-> Hash function for \texttt{HashMap}: SHA256 \texttt{string}
    $\rightarrow$ 32-bit \texttt{int}. (4~LOC)

  \item<3-> Define and spawn a local Troupe thread that owns the
    \texttt{HashMap}. (21~LOC)
...
HashMap.findAll m auth k
  ...
  \item<4-> Define and export \texttt{set} and \texttt{get} functions callable
    from the TCB. (8~LOC)
...
fun set (key, value) = send(thread, (SET, key, value))
  ...
  \item<5-> Use \texttt{sandbox} builtin to guard against timing attacks.
    (14~LOC)
  \end{itemize}
\end{frame}
...
\begin{frame}[t]
  ...
  \vspace{-20pt}
  \begin{align*}
      C &\xrightarrow{f_{\texttt{Mod}}@\{C\}} B \implies \text{miss}
    & A &\xrightarrow{g_{\texttt{Mod}}@\{\}}  B \implies \text{miss}
    & D &\xrightarrow{h_{\texttt{Mod}}@\{D\}} B \onslide<2->{\implies \text{hit}}
  \end{align*}

  \onslide<3->{
    \textbf{``Specification'':}

    \vspace{-6pt} An IFC-aware (\texttt{Hash})\texttt{Map} is a single data
    structure with public and secret key-value pairs which provides a IFC-aware
    projections of its \texttt{insert} and \texttt{remove}\footnotemark\
    history:
...
\begin{frame}
  ...
  \item {\bf Peer-to-peer layer:}
    ...
    Extend with \texttt{miss} and \texttt{miss reply} messages which are always
    sent by value.
