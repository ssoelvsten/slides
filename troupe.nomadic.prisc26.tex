...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      \onslide<2-> {
        \node[shape=rectangle, draw=black] at (0,-1.71)
        (M) {\texttt{M}};

        \draw[->]
          % C -> B
          (M) edge[bend right=10] node[below] {$g_{\texttt{auth}}$} (B)
        ;
      }
    \end{tikzpicture}
...
\begin{frame}[t]
  ...
  \footnotetext[1]<3->{In this particular use case, we actually only need public
    ( \texttt{@\{\}} ) \texttt{remove} operations.}
\end{frame}
...
\begin{frame}[plain,noframenumbering]
  \begin{center}
    ...
    \onslide<2->{
      ...
      By piggy-backing on the IFC-aware language itself!
    }
  \end{center}
\end{frame}
