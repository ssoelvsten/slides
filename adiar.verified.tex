...
bdd_eval' ((i t e)::ns': List[Node]) (tgt: Uid) (x: $\N \rightarrow \B$) =
    ...
    else case if x i.level then t else e of
           Leaf(b)    $\rightarrow$ b
...
combine_paths' (pq : PQ<Req>) (tgt : Uid) ((sacc, lacc) : $\N \times \N$) =
    case pq.top() of
      None              $\rightarrow$ (sacc, lacc, pq)
      Some Req tgt' s $\ell$ $\rightarrow$ if tgt' $\neq$ tgt
                           then (sacc, lacc, pq)
...
forward_paths (pq : PQ<Req>) (ptr : Ptr) (s : $\N$) ($\ell$ : $\N$) =
    case s, ptr of
      0, _          $\rightarrow$ (0, pq) *@{\color{gray} (* \emph{Well formed} $\not\Rightarrow$ fully connected *)}@*
      _, Leaf False $\rightarrow$ (0, pq)
      _, Leaf True  $\rightarrow$ (s$\cdot 2^{\vc - \ell}$, pq)
      _, Node tgt   $\rightarrow$ (0, pq + {(tgt, s, $\ell$)})
  \end{lstlisting}
