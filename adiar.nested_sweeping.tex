...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.59\linewidth}
      \only<2>{
        \begin{definition}[Lee~'58, Akers~78]
        \end{definition}
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=1.25]
      ...
      \draw[->, color=black, thick]
        (6.5,0.5) -- ++(0.5,0)
        node[pos=2.1]{\large $\exists x_i.\ \phi$}
      ;
    \end{tikzpicture}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{$\exists x_1 .\ \phi \equiv \phi[x_1 \mapsto \bot] \lor \phi[x_i \mapsto \top]$ : \texttt{Apply}}
...
\begin{frame}
  \frametitle{$\exists x_1 .\ \phi \equiv \phi[x_1 \mapsto \bot] \lor \phi[x_i \mapsto \top]$ : \texttt{Reduce}}
...
\begin{frame}
  \begin{center}
    {\bf {\huge Adiar:}\\{\LARGE Binary Decision Diagrams in External Memory}}

    {\large Steffan Christ Sølvsten, Jaco Van de Pol,\\Anna Blume Jakobsen,
      and Mathias Weller Berg Thomasen}

    % {\footnotesize Department of Computer Science\\Aarhus University}

    {\footnotesize March 2022}
  \end{center}
\end{frame}
...
\begin{frame}
  \begin{center}
    {\LARGE\bf Multi-variable Quantification of BDDs\\in External Memory using Nested Sweeping}

    {\large Steffan Christ Sølvsten and Jaco Van de Pol}
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=1.25]
      ...
      node[pos=1.2]{\large $\exists \vec{x}.\ \phi$};
...
\begin{frame}
  \frametitle{$\exists \vec{x}.\ \phi$ \quad $\vec{x} = \{ x_0, x_1 \}$}
...
\begin{frame}
  ...
  {\bf Theorem~(S{\o}lvsten et al.~'26)}
